...
export default function Deployments() {
    ...
    return (
        <>
            ...
            <section className="cyber-grid border-primary/15 bg-surface rounded-3xl border p-8 shadow-[0_0_22px_rgba(204,255,0,0.08)]">
                ...
                <div className="grid gap-3">
                    {cms.deploymentSteps.map((item, index) => (
                        <div key={item} className="flex items-center gap-4 rounded-2xl border border-white/5 bg-black/45 p-4">
                            <span className="border-primary/20 text-primary flex h-8 w-8 items-center justify-center rounded-lg border text-[10px]">
                                0{index + 1}
                            </span>
                            <span className="text-on-surface-variant text-[11px] font-bold tracking-widest uppercase">{item}</span>
                        </div>
                    ))}
                </div>
            </section>
